...
int Model::compile(std::vector<Tensor> &inputs, std::vector<Tensor> &outputs,
  ...
  for (auto &[leaf_name, leaf_info] : additional_leaves) {
    ...
    // Carry the leaf's tensor dtype through to the input layer. Without
    // this, every leaf placeholder is created with the model's default
    // activation dtype (FP32), and any externally-bound buffer that
    // disagrees — most visibly the FP16 KV-cache slabs on Android builds
    // with ENABLE_FP16, or the UINT16 cache on Linux — gets rejected at
    // bind time with "cache placeholder dtype mismatch for layer N".
