...
|---|---:|---:|---:|---:|---:|
| **expert FFN 루프** (한 MoE 레이어 = expert 32개 × [M×2048×3584 → SwiGLU → M×1792×2048], M 합 1776, 39 GFLOP, 크기는 §2.1. 라우터·gather·scatter 제외) | 34.6 ms | 26.3 ms | **15.2 ms** | **2.3×** | 1.7× |
| ↳ 그중 순수 행렬 유닛 시간 | (GEMM 안에 양자화·f32 출력이 섞여 분리 불가) | (같음) | HMX 명령 9.25 + acc_read 2.93 = **12.2 ms** | (2.8×, 범위가 다름) | (2.2×) |
| **MoE 레이어 전체** (라우터·양자화·전송 포함) | 41.1 ms | 31.8 ms | **18.4 ms** | **2.2×** | 1.7× |
| **prefill 전체** (444 토큰, 24층, 3회 최솟값) | 324 TPS (1371 ms) | 279 TPS (1590 ms) | **523 TPS (848 ms)** | **1.6×** | 1.9× |
| **decode** (토큰 1개, 512 생성) | **48 TPS** | 35 TPS | 20 TPS | **0.42×** (CPU가 빠름) | 0.56× |

**세 열의 출처.** 원본 CPU 열만 이번(09-17)에 8스레드로 새로 쟀고, 나머지 CPU 열은 예전 값이다.

| 열 | 모델 · 경로 | 어디서 | 언제·조건 |
|---|---|---|---|
| CPU 원본 흐름 | `lfm2.5-8b-a1b-q40`: 전 레이어 Q4_0, MoE expert는 ggml Q4_0 GEMM (PR 4264 그대로, `moe_engine` 없음) | §6.1 결과: 비프로파일 3회 1847/1371/1470 ms, decode 48.1/47.2/48.7 TPS(열 정상). `NNTR_M0_PROFILE=1` 22줄 평균 | **2026-09-17, 8스레드** |
...
| NPU | `…-q40-qs4cx-wh`: MoE만 QS4CX_WH, 아레나 | 문서 47 §22.2: host 17.5 ms/콜 + ARM ≈ 0.9. 비프로파일 3회 최솟값 848 ms | 2026-09-16, 8스레드 |
...
| **ffn** (Q4_0 GEMM gate_up → SwiGLU → GEMM down, 32 expert) | **34.64** | 15.2 (HMX 12.2 + 에필로그·requant·DMA 노출 3.0) |
| route (라우팅 가중치 곱) + scatter | 1.46 + 1.23 | DSP scatter 잡 (숨음) |
| **레이어** | **41.1** (38.7–43.6) | 18.4 |

ffn 34.6 ms = 39.1 GFLOP → **1.13 TFLOPS**. KleidiAI QS4CX(26.3 ms, 1.49 TFLOPS)보다
ggml Q4_0이 prefill에서 24% 느리다. 반대로 decode는 Q4_0 48 TPS가 QS4CX 35 TPS보다 빠른데,
...
decode 47–49 TPS로 세 실행 모두 열 게이트(30) 통과. 텍스트는 세 실행 동일(긴 `<think>` 뒤
요약 시작, 512 토큰에서 잘림 — QS4CX/WH 모델의 3문장 요약과는 다른 출력이고, 가중치 값이
다르니 당연하다). `[M0-PROF]` 22줄 평균은 §1의 분해 표: 레이어 41.1 ms, ffn 34.6 ms.
...
| 2 | **NPU, `NNTR_M0_PROFILE=1` 1회**: `…-q40-qs4cx-wh`, 8스레드 | NPU 레이어 18.4는 host 17.5 + ARM 추정 0.9. 같은 계측기(`us=`, `ffn=`)로 잰 값이 있어야 CPU 열과 같은 줄에 놓인다 | `NNTR_NUM_THREADS=8 NNTR_M0_PROFILE=1 <run> 2>&1 \| grep M0-PROF` |
| 3 | (선택) CPU Q4_0 4스레드 1회 | 09-08의 1590 ms와 같은 조건으로 맞춰 보는 대조. 스레드 효과의 크기 | `<run>` (env 없이) |

1·2가 끝나면 §1의 표는 세 열 모두 같은 주, 같은 스레드 수, 같은 계측기가 된다. 그 밖에
