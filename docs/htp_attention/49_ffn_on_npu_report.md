...
### 3.2.3 콜 하나의 전체 분해 (프로파일 열 전부)

`NNTR_HTP_PROFILE=2`, 2026-09-16, 22콜 평균. MoE 레이어 하나, prefill 444 토큰.

| 열 | ms | 비중 | 무엇 |
|---|---:|---:|---|
| **mm** | 9.25 | 52.9% | HMX 행렬곱 명령 발행. 49만 회 |
| **acc** | 2.93 | 16.8% | accumulator → VTCM 읽어내기. 읽는 동안 HMX 정지 |
| **transport** | 2.08 | 11.9% | FastRPC 왕복 + 공유 버퍼 캐시 유지 |
| **requant** | 1.06 | 6.1% | f32 → u8. 동기라 통째로 노출 |
| **dequant** | 0.85 | 4.9% | int32 → f32 중 **숨기지 못한 몫** |
| **stage** | 0.36 | 2.1% | 입력·출력 DMA 복사 + `out_c` 0채우기, 셋 합쳐서 |
| **quant** | 0.26 | 1.5% | 행별 scan + pack 기다림의 노출분 |
| rest | 0.19 | 1.1% | 위 어디에도 안 잡힌 나머지 |
| drain | 0.16 | 0.9% | gate_up DMA 대기 노출분 |
| gather | 0.14 | 0.8% | 활성화 블록 DMA 대기 + scale/zp 꺼내기 |
| drain_dn | 0.06 | 0.3% | down DMA 대기 노출분 |
| alloc | 0.06 | 0.3% | 스크래치 확보. 첫 콜 이후 0 |
| push | 0.05 | 0.3% | DMA 디스크립터 쓰기 |
| scatter | 0.05 | 0.3% | scatter 잡 회수 노출분 |
| **host 합** | **17.50** | 100% | ARM이 잰 FastRPC 왕복 |

여기에 ARM 쪽 staging memcpy 0.45 × 2 = **0.9 ms**가 콜 밖에 따로 붙어 `M0` `ffn` 17.4가 된다.

**성격별로 묶으면:**

| | ms | 비중 |
|---|---:|---:|
| 계산 (HMX) — mm + acc | 12.19 | 69.6% |
| 형식 변환 (HVX 노출분) — requant + dequant + quant | 2.17 | 12.4% |
| 데이터 이동 — transport + stage + drain + gather + push | 2.85 | 16.3% |
| 기타 — rest + alloc + scatter | 0.30 | 1.7% |

**읽을 때 주의할 것.** `dequant`·`scatter`·`drain`·`gather`는 **일의 양이 아니라 노출된
시간**이다. 실제 작업량은 훨씬 크지만 워커들이 HMX 뒤에서 처리해 대부분 안 보인다. 반대로
`mm`·`acc`·`requant`는 진짜 그만큼 걸린 시간이다. requant가 dequant보다 블록당 절반만
하는데도 더 비싼 이유가 이것이다 — 숨을 곳이 없다(§4.5 B-5의 35번).

**전체 prefill에서의 비중.** 17.50 × 22층 = 385 ms, ARM staging 20 ms를 더해 **405 ms**로
prefill 848 ms의 **48%**다. 나머지 443 ms는 conv·attention·norm·lm_head가 ARM에서 도는
시간이고, CPU 실행에서도 같은 크기(1329 − 22 × 40.9 = 429 ms)로 나온다(§3.2.1).
...
### 4.0 메모리 계층 — 데이터가 거치는 세 단

흐름을 읽기 전에 알아야 할 것. **DSP 칩 안에 있는 메모리는 두 종류뿐이고, 나머지는 전부
DDR이다.** 아래에서 DSP 힙과 ION 버퍼는 같은 DDR이고 매핑 속성만 다르다.

```
DSP 칩 안
├─ 스칼라 코어 + HVX 유닛 × 4 + HMX 유닛
├─ L1 / L2 캐시          ← 자동. DDR에서 최근 읽은 내용을 붙들고 있다
└─ VTCM 8 MiB            ← 수동. DMA로 넣어야 들어온다. HMX는 오직 여기만 읽는다

DDR (칩 밖, 폰의 주 메모리)
├─ ARM 프로세스 힙       ← 텐서. DSP는 못 본다
├─ ION 버퍼 3.6 MB × 2   ← ARM과 공유하는 창문. DSP 쪽 매핑은 캐시가 없다
├─ DSP 힙 12.8 MB        ← act_c, out_c, act_ah 같은 작업 공간. 캐시를 탄다
└─ ION 아레나 3.9 GB     ← expert 가중치 전부. 로드 때 채우고 끝
```

**VTCM만 특별하다.** 캐시가 아니라 주소가 있는 진짜 SRAM이라 알아서 채워지지 않는다.
대신 칩 안이라 빠르고, HMX가 읽을 수 있는 유일한 곳이다.

**활성화 3.6 MB가 거치는 세 번의 이동** — 각각 이유가 다르다:

| | 어디서 → 어디로 | 수단 | 왜 |
|---|---|---|---|
| ① | ARM 텐서 힙 → ION 버퍼 | ARM memcpy, 0.45 ms | DSP에게 보여주려고. 평범한 힙은 콜마다 페이지 고정·등록이 필요하다(§5.0) |
| ② | ION 버퍼 → DSP 힙 `act_c` | DSP DMA, `stage`에 포함 | ION은 DSP 매핑에 캐시가 없다. 이 데이터는 scan과 pack에서 **두 번** 훑으므로 캐시 타는 곳으로 한 번 옮긴다. memcpy로 하면 3.6 ms, DMA로는 사실상 공짜 |
| ③ | DSP 힙 `act_ah` → VTCM `act_off` | DSP DMA, 블록마다 128 KB | HMX가 VTCM만 읽으니까. 팩된 활성화 전체는 6.0 MB인데 VTCM은 가중치에 5.25 MiB를 이미 내줬다 |

출력은 정확히 반대 순서로 돌아온다: VTCM → (scatter가) DSP 힙 `out_c` → ION 버퍼 → ARM 텐서.

**왜 `act_c`를 그냥 VTCM에 두지 않나.** 안 들어간다. VTCM 8 MiB 중 가중치가 5.25 MiB를
쓰고, 팩된 활성화는 6.0 MB다(§4.6). 그래서 활성화는 DDR에 두고 64행씩 128 KB만 퍼 나른다.
그 퍼 나르기가 흐름도의 `gather`이고, 다음 블록 것을 미리 걸어두어 계산 뒤에 숨긴다.
...
### 5.0 기준선(181 TPS)에 이미 들어 있던 구조적 결정

아래 §5.1의 표는 181 → 523의 경로다. 그 **181 자체가** 이미 네 가지 결정 위에 서 있다.
전부 9월 15일 이전이고, 없었으면 NPU 경로는 CPU 근처에도 못 갔다. 넷 다 "계산을 빠르게"가
아니라 **"ARM과 DSP 사이로 바이트를 덜 보내기"** 다.

| | 무엇을 했나 | 왜 | 실측 |
|---|---|---|---|
| **레이어당 콜 하나** | expert마다 행렬곱마다 따로 부르던 것(레이어당 64콜)을 레이어 전체를 한 콜로 | 콜 하나에 고정비가 붙는다. 64번이면 그 고정비만 레이어당 14.76 ms | transport **14.76 → 2.1 ms/레이어** (문서 44 §13.1, 46 resident kernel) |
| **활성화·출력을 ION 버퍼에** | 텐서 힙 포인터를 그대로 넘기던 것을, 프로세스 수명 ION 버퍼(`act_buf_`/`out_buf_`, 3.6 MB씩)에 복사해서 넘기게 | 평범한 힙은 드라이버가 **콜마다** 페이지를 고정하고 DSP 주소 테이블에 등록했다 뗀다. ION은 그 등록이 유지된다 | 페이로드 경로 **~155 MB/s → 44.6 GB/s** (문서 34 §4 F). 대신 memcpy 0.45 ms × 2가 새로 생긴다 — 그래도 남는 장사 |
| **가중치를 ION 아레나에 상주** | expert 가중치 3.9 GB를 로드 때 아레나에 넣고, 콜에는 핸들만 | 안 그러면 콜마다 5.25 MiB × 32를 실어 보내거나 DSP 힙에 구워야 했다(천장 1.89 GB로 모델이 안 들어감) | 가중치가 **와이어를 아예 안 탄다**. 등록 때도 weight당 scale·colsum·bias 4 KB만 건넌다 |
| **poll QoS** | FastRPC 지연 모드를 `RPC_POLL_QOS`(latency 100 us)로 | 기본 모드는 DSP 응답을 인터럽트로 기다려서 깨어나는 시간이 들쭉날쭉했다 | transport 편차 **90 ~ 3,900 us → 안정** (문서 46, `htp_backend.cpp:64–71`) |

**ION 버퍼의 맞바꿈이 핵심이다.** 복사를 없앤 게 아니라 **복사를 하나 추가해서 더 비싼
일을 없앤 것**이다. 콜마다 3.6 MB × 2를 memcpy(0.9 ms)하는 대신, 콜마다 페이지를 고정하고
DSP 주소 테이블에 등록했다 떼는 일을 없앴다. 코드 주석이 "이 맞바꿈은 반드시 기기의
transport 열로 확인해라, 이겼다고 가정하지 마라"라고 못박아둔 이유이기도 하다
(`htp_compute_ops.cpp:1176–1186`).

남은 0.9 ms를 마저 없애려면 텐서 자체를 ION에 할당해야 한다 — TensorPool 전체를 건드리는
일이라 안 했다(§7).
