...
**MoE FFN만으로는 1.26배가 천장이다.** 못 줄이는 바닥이 mm 5.9 + acc 2.75 = 8.7 ms인데 CPU FFN 전체가 26.3 ms다. **HMX 행렬곱 자체는 CPU보다 4.5배 빠르다** — 포장(quant/dequant/drain/transport) 17.7 ms가 연산 5.9 ms의 3배인 것이 전부다. 포장을 없애려면 값이 레이어 사이에 DSP에 머물러야 하고 그것이 Phase 5(§13.4)다. 2~3배를 원하면 Phase 5 말고 길이 없다.
