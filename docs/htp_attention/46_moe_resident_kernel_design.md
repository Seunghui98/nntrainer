...
### 35.3a 1차 결과 (2026-09-15, 기기) — **레이아웃이 풀렸다**

```
bijection=yes                       ← 1024 슬롯 전부 정확히 한 번씩
arm_convert_us=23342  mbps=157.2    ← ARM 변환 속도
bad_bytes=3653494 of 3670016        ← sdkl(ARM) vs DSP bake, 2048x3584
tiles_mismatched=0 of 4             ← 64x64에서 sdkl을 자기 자신과 비교한 것
```
...
앞 세 항이 전부 짝수라 마지막 항이 곧 니블 반쪽이다:

```
byte(r,c) = (c/8)*128 + r*4 + (c%4)        nibble = (c/4)%2
```

**한 바이트는 같은 행에서 4칸 떨어진 두 열**(c와 c+4)을 담고, c의 비트2가 하위/상위를
고른다. 32행 × 4열 × 4열그룹 = 512바이트, 남는 자리 없음. 1024칸 전부 이 식과 일치한다.
`whSlot`/`whPackReference`로 코드에 들어갔다 — 2 KB 데이터 블롭이 아니라 한 줄짜리
인덱스 계산이므로, §35.6의 라이선스 항목은 사실상 비켜간다.

**ARM 벤더 함수는 DSP와 안 맞았다 (99.5%).** 단, `tiles_mismatched=0`은 sdkl 출력을
*표로 재구성한 sdkl*과 비교한 것이라 이 불일치를 볼 수 없었다 — 32×32 **정사각**에서
표를 뽑았기 때문이다. 유력한 원인은 **인자 순서**: `hexkl_macro_i4_rm_to_i4_wh`는
`(n_col, n_inner)`인데 `sdkl_cpu_i4_rm_to_i4_wh`는 문서상 `(wt_rows, wt_cols)`다.
정사각 입력은 둘을 구분할 수 없다.

**교훈**: 대칭인 shape로 비대칭 가정을 검증할 수 없다. 표 추출은 32×32로 맞았지만
검증은 처음부터 비정사각이어야 했다.

**조치**: 기준을 벤더가 아니라 **DSP로 바꿨다.** `WhPackReferenceMatchesDspBake`가
`whPackReference`를 2048×3584에서 `weight_bake_export`와 직접 비교한다 — 오프라인
양자화기가 답해야 하는 질문이 정확히 이것이고, sdkl이 어느 인자 순서를 원하는지는
별개 문제로 옆에 같이 찍는다(`sdkl_rows_cols_bad_bytes` / `sdkl_cols_rows_bad_bytes`,
그리고 비정사각 64×128에서 `nonsquare_sdkl_*_bad`).
...
### 35.4 남은 작업 — 바이트를 런타임까지 옮기기

`nntrainer/tensor/htp_backend/htp_wh_layout.h` (`whSlot`/`whPack`/`whBytes`)가
확정된 패커다. 테스트가 이 헤더를 include하므로 **모델 파일을 만드는 코드와 기기가
검증하는 코드가 같다** — 사본이 아니다. `WhPackReferenceMatchesDspBake`가 그 고리이고,
계속 통과해야 한다. 하드웨어 레이아웃의 복사본이라, 여전히 맞는지는 기기만이 말할 수 있다.

**남은 결정 하나**: WH 바이트가 어떤 경로로 런타임에 도달하는가.

| | 형태 | 런타임 변경 |
|---|---|---|
| **W1** | `quantize_stream`에 `QS4CX_WH` dtype. `.bin` 안에 WH 니블 + scale(4N) + **colsum(4N)** | 텐서 로더가 새 dtype을 알아야 하고, **colsum이 ops seam을 통과할 길**이 필요하다(`gemm_qs4cx_moe_layer_fp32`는 지금 data와 scale만 받는다) |
| **W2** | `.bin`은 QS4CX 그대로 두고, 양자화기가 **옆에 WH 파일들을 같이 쓴다**(§34의 그 형식) | **없음.** §34 아레나 경로가 이미 그 파일을 읽어 아레나에 넣는다. 항상 hit이고 bake는 영영 안 돈다 |

W2가 코드를 한 줄도 안 늘린다. 다만 키가 런타임에서 계산하는 소스 해시라 양자화기가
그걸 재현해야 하고, 형태가 "캐시 디렉터리"로 보인다 — 실제로는 오프라인 변환 산출물이다.

W1이 정공법이고 모델 파일 하나로 끝나지만, colsum 배관이 ops seam을 건드린다.
...
## 38. 벽이 안 움직였다 (2026-09-15, 기기) — 호스트 RAM이 아니다

§37을 넣고 돌린 결과: **1170. 한 개도 안 움직였다.**

```
weights registered : 1170  (/1408)
register FastRPC   : 106.7 ms  → 0.09 ms/weight
rpcmem/ION         : 1170/1170
FATAL: HTP arena is full; cannot register a 2048x3584 weight
```

### 38.1 1170이 무슨 숫자인가

gate_up과 down이 번갈아 오므로 585개씩이고,

```
585 × 3,670,016 B (2048×3584)  = 2047.5 MiB
585 × 1,835,008 B (1792×2048)  = 1023.75 MiB
                                 --------------
                                 3071.25 MiB = 정확히 3.00 GiB
```

**1 GiB 청크 3개는 붙었고 4번째가 거부됐다.** 모델은 3.61 GiB가 필요하다(22 × 32 ×
5.25 MiB). 17% 모자란다.

### 38.2 이게 뜻하는 것

madvise로 3 GB 가까이 반납했는데 한계가 **1바이트도** 안 밀렸다. 호스트 RAM 압력이
원인이었다면 숫자가 흐릿하게라도 움직였을 것이다. 안 움직였다 = **한계는 호스트
메모리가 아니다.** §36.2가 세운 진단("피크 7.8 GB라서 ION이 거부")이 틀렸다.

경계도 좁혀졌다. 256 MiB × 8 = 2 GiB에서 멈췄던 건 `NNTR_HVX_MAX_ARENAS`(개수)가
먼저 걸린 것이고, 1 GiB 청크로 개수 제약을 피하니 **3 GiB에서 바이트 제약**이
나왔다. 개수가 아니라 총량이다.

### 38.3 아직 모르는 것 — 네 호출 중 어디인가

`newChunk`는 네 군데에서 `return false`를 했고 전부 같은 메시지로 죽었다:

| 실패 지점 | 뜻 | 고치는 방향 |
|---|---|---|
| `rpcmem_alloc` | 호스트 ION heap | ARM 사본을 더 줄인다 |
| `rpcmem_to_fd` | 드라이버 | 별개 버그 |
| `fastrpc_mmap` | **DSP 주소공간 / 드라이버 매핑 테이블** | 호스트 메모리와 무관 |
| `arena_attach` | DSP가 거부 (`MAX_ARENAS`, `HAP_mmap_get`) | DSP 쪽 상수 |

셋째가 제일 유력하지만 **추측이고**, 각각 할 일이 전혀 다르다. §33.2가 남긴 교훈이
정확히 이거다 — 숫자에 맞는 가설이 곧 원인은 아니다.

### 38.4 계측 (2026-09-15, 코드 완료·미측정)

- `newChunk`의 네 경로가 각각 **어느 호출이 어떤 코드로** 거부했는지 문장으로 남긴다.
  `arena_attach`는 hex도 찍는다(`AEEStdErr`가 `__hexagon__`에서 0x80000400을 더하므로
  10진수만으로는 `AEE_EBADSTATE`가 안 보인다, §32.10).
- 성공한 청크마다 `[HTP] arena chunk N: size, dsp_id, mapped total, RSS before→after`.
- 죽을 때 throw가 **실패 이유 + mapped MiB + 청크 수 + RSS**를 같이 들고 나온다.

RSS가 핵심이다. `MADV_DONTNEED`는 실제로 회수를 못 해도 실패를 보고하지 않는다.
RSS가 3.9 GB 근처면 §37의 페이지 반납이 애초에 안 먹은 것이고(그러면 풀을 `mmap`으로
...
## 41. 천장은 3 GiB가 아니었다 — 정렬 낭비였다 (2026-09-15, 기기)

§40.3 프로브가 깨끗한 프로세스에서 잰 값:

```
U8I4_FIELD field=mmap_ceiling_mib  value=3840      ← 3.75 GiB
U8I4_FIELD field=last_mmap_rc      value=0x00000001
U8I4_FIELD field=heap_weights      value=52   (182 MiB)
U8I4_FIELD field=heap_stop_rc      value=0x80000402  (= AEE_ENOMEMORY + kDspOffset)
U8I4_FIELD field=covers_overflow   value=no
```

**모델이 필요한 건 3696 MiB다. 3840 > 3696 — 들어간다.**

### 41.1 두 숫자를 동시에 설명하는 것은 하나뿐이다

| | 청크 | 도달 | 그 다음 |
|---|---|---|---|
| 앱 | 1024 MiB | **3072** | 512·256·128·64 전부 거부 |
| 프로브 | 256 MiB | **3840** | — |

평평한 바이트 예산이면 앱에서 3840−3072 = 768 MiB가 남아 있어야 하고 512 MiB가
붙었어야 한다. 안 붙었다. **범프 할당자 + 크기만큼의 정렬**이면 둘 다 설명된다:

```
PD가 앞쪽 ~256 MiB 사용
1 GiB 청크: 커서를 1G로 올림 → 1G, 2G, 3G → 커서 4G = 주소공간 끝.
            [256M, 1G)의 768 MiB는 커서가 지나가서 영영 못 쓴다. 64 MiB조차.
256 MiB 청크: 256M부터 15개가 딱 4G까지. 낭비 0.
```

`heap_stop_rc`가 이걸 확증한다. 3840 MiB를 매핑한 채로 heap이 182 MiB만 주고
`AEE_ENOMEMORY`를 냈다 — 3840 + 182 + PD ≈ **4096 MiB**. heap과 매핑은 **같은 4 GB
주소공간**을 나눠 쓴다. 따로 찾을 두 번째 예산은 없다. **낭비를 그만두는 것만 있다.**

### 41.2 수정 — 한 줄

`kArenaChunkMax` 1 GiB → **256 MiB**. 3696 MiB = 15청크, `NNTR_HVX_MAX_ARENAS=32`
안에 들어간다. 여유는 144 MiB이고, `place()`가 **모든** 청크를 훑어 자리를 찾는 것이
그 여유를 지킨다 — 1.75 MiB짜리 down은 3.5 MiB짜리 gate_up이 못 들어가는 꼬리에
들어간다.

§36.2부터 §40까지 세 번의 진단이 전부 틀렸고(피크 메모리 → 호스트 ION → heap 별도
예산), 맞은 것은 **깨끗한 프로세스에서 같은 것을 다른 크기로 재본 것**뿐이다.
§33.2의 교훈이 그대로 반복됐다: 숫자에 맞는 가설이 곧 원인이 아니다.
...
| 타입 | 합 | % |
|---|---:|---:|
| `lfm2_moe` | 32.4 s | 59.9 |
| **`tie_word_embeddings`** | **13.2 s** | **24.4** |
| `fully_connected` | 4.8 s | 8.8 |
| `mha_core` | 1.7 s | 3.1 |
| 나머지 전부(rms_norm, addition, conv, split, swiglu, …) | ~1.0 s | 1.8 |

13.2 s 중 13.17 s가 **`output_of_causallm`** 한 노드다 — 토큰당 평균 **25664 us**.
`embedding0`은 같은 타입인데 5214 us 전체다(룩업이라 싸다).

### 43.1 원인은 커널이 아니라 양자화 옵션이다

`quantize_stream.cpp:1331` `--embd_dtype` 기본값 = **FP32**, `:1430` `--lmhead_dtype`
기본값 = embedding dtype. 지금 모델은 둘 다 안 준 채로 만들어졌다.

```
vocab 65536 × hidden 2048 × 4 B = 537 MB  ← 토큰마다 읽는다
537 MB / 25.7 ms = 20.9 GB/s              ← ARM DDR 실측 대역폭 그대로
```

산술이 맞는다. **커널이 느린 게 아니라 537 MB를 읽고 있다.**

### 43.2 고치는 법 — 재양자화 한 번, 코드 0줄

```
--lmhead_dtype Q4_0   537 MB -> 75 MB,  25.7 ms -> ~4 ms
```

- **~11 s 절감 = 전체의 20%.** decode 10.3 -> 약 13 TPS
- 피크 메모리 −460 MB
- embedding(`embedding0`)은 FP32로 둬도 된다 — 5.2 ms 전체다. 메모리를 더 줄이려면
  `--embd_dtype`도 내리면 되지만 속도 효과는 없다
- 정확도: lm_head 양자화는 로짓에 직접 닿는다. Q4_0이 거칠면 `Q6_K`가 중간값
  (537 -> ~110 MB, ~8 ms). **출력 텍스트로 확인해야 한다**

### 43.3 §42.4 순위 수정

| | 항목 | 절감 | 비용 |
|---|---|---:|---|
| **H** | **`--lmhead_dtype` 재양자화** | **~11 s** | **코드 0줄** |
| B | `gather` 고정비 | ~8.7 s | 중 |
| D | decode DMA 10.8 -> 30 GB/s | ~5 s | 큼 |
| E | `acc`가 M 존중 | ~2.9 s | 중 |
| C | prebound invoke | ~2.3 s | 중 |
| | `fully_connected` 4.8 s | ? | 아직 안 봄 |

H가 가장 크고 가장 싸다. §42.4에서 A를 먼저 하자고 한 이유가 이것이다 — B부터
갔으면 8.7 s를 위해 커널을 고치면서 코드 0줄짜리 11 s를 못 보고 지나쳤다.

### 43.4 주의 — `--profile` 빌드는 prefill을 왜곡한다
...
## 45. 스레드 판별 결과 (2026-09-15, 기기) — 원인이 둘이었다

§44.3의 repack을 쓰기 전에 `NNTR_NUM_THREADS`를 4(기본,
`hardware_concurrency()/2`)에서 8로 올려 비교했다.

| | threads=4 | threads=8 | 차이 |
|---|---:|---:|---:|
| `output_of_causallm` | 13.17 s (24.4%) | **7.60 s** (13.8%) | **−5.57** |
| `fully_connected` | 4.77 s (8.8%) | **11.48 s** (20.9%) | **+6.71** |
| `lfm2_moe` | 32.39 s | 32.52 s | ~0 |
| generation | 49595 ms | 50961 ms | **+1.4 (손해)** |

### 45.1 스레드 가설은 맞았다 — 그리고 답이 아니다

lm_head는 42% 빨라졌는데 FC가 2.4배 느려져 **순손해**다. FC 노드의
avg/min을 같이 보면 원인이 나온다:

| 노드 | avg 4→8 | min 4→8 |
|---|---|---|
| `layer7_conv_in_proj` | 213 → **1302** | 143 → 143 |
| `layer0_conv_in_proj` | 320 → **854** | 163 → 136 |
| `layer21_wq` | 142 → **479** | 84 → 67 |

**min은 안 움직이고 avg만 폭발한다.** 빠른 경로는 그대로 빠르다는 뜻이고,
big.LITTLE 오버서브스크립션의 서명이다 — decode의 FC는 2 MB짜리 일인데 8갈래로
쪼개면 little 코어에 떨어진 조각을 나머지가 기다린다. `thread_manager.h:107`의
`@todo use performance core only`가 그 자리에 그냥 있다.

### 45.2 레이아웃 문제는 그대로다

```
lm_head @ 8 threads : 75.5 MB / 14.8 ms = 5.1 GB/s
repack된 FC @ 4     :  7.1 MB / 320 us  = 22  GB/s
```

스레드를 두 배로 줘도 **4배 이상 느리다.** §44.2의 진단은 유효하고, 두 원인이
곱해져 있었을 뿐이다.

### 45.3 결론 — repack을 하고, 스레드는 건드리지 않는다

기본 4스레드에서 레이아웃만 고치면 75.5 MB를 22 GB/s로 = **~3.4 ms**,
13.17 s -> **~1.75 s. 11.4 s 절감, FC 회귀 없음.**
