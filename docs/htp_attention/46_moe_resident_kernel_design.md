...
### 33.2a 원인 — bisect 전에 산술이 답했다 (2026-09-14)

**커널 버그가 아니라 테스트가 커널의 전제를 어긴 입력을 넣고 있었다.**

`moe_scatter_worker`는 블록을 행 단위로 쪼개면서 주석에 전제를 적어놨다:
"한 토큰은 서로 다른 expert k개를 고른다 → 한 expert 블록 안에서 row_index는
전부 다르다 → 두 워커가 같은 출력 행을 건드릴 일이 없다."

`MoeLayerMatchesTwoCallReference`의 생성기는 **복원추출**이었다. expert 0에만
같은 행이 13번 중복됐고, 그 중복들은 서로 다른 워커 슬라이스로 갈라져
같은 `out_c` 행을 동시에 read-modify-write 했다. 실제 top-k 라우팅은
절대 만들지 않는 입력이다.

숫자가 정확히 맞는다 (LCG가 결정적이라 기기 없이 재현했다):

| | 행 수 | 원소 | 비율 |
|---|---|---|---|
| 1개 expert만 쓴 dst 행 | 79 | 161792 | 39.5% |
| 2개 | 22 | 45056 | 11.0% |
| **≥3개** | **12** | **24576** | **6.00%** |
| 관측된 `bad_elems` | 9.81행 분 | 20097 | **4.91%** |

- 2개짜리(11%)는 전부 일치한다 — `a+b == b+a`는 부동소수점에서도 정확히 성립.
- 3개 이상이면 결합법칙이 깨지므로 순서가 바뀌면 1 ULP가 난다. 상한 6.00%,
  관측 4.91% = 그 12행의 82%.
- 첫 불일치가 정확히 1 ULP였던 것도 이 설명하고만 맞는다.

`corrected = acc - zp*colsum`은 **정수 범위 안에서 정확**하다(양쪽 다 2^24 미만).
그래서 입력이 같으면 dequant는 비트 단위로 같을 수밖에 없고, 1 ULP가 났다는 건
더해지는 **순서**가 달랐다는 뜻이다. 이게 "1 ULP는 레이스가 아니다"를 뒤집는
지점이다 — 누락된 덧셈(lost update)은 큰 오차를 내지만, 순서만 바뀐 덧셈은
정확히 1 ULP를 낸다.

**조치**

1. 테스트 생성기를 비복원추출로 바꿔 실제 라우팅과 같게 했다. 블록 내 중복 0,
   ≥3-expert 행은 2개로 커버는 유지된다.
2. 실패해도 위치를 짚도록 세 필드를 추가했다: `rows_with_k_experts`,
   `bad_on_single_expert_rows`, `max_ulp`.
   - 전부 multi-expert 행 + `max_ulp=1` → 누적 순서 문제.
   - `bad_on_single_expert_rows > 0` → matmul이나 quantizer. **이 때만 bisect.**
3. 커널 진입점(`row_index[i] >= M` 검증 옆)에 전제를 적었다. 범위는 검증하지만
   중복은 검증하지 않는다 — expert당 M 비트맵 스캔 비용을 라우팅이 이미
   보장하는 걸 다시 확인하는 데 쓰지 않는다.

**아직 확인 안 됨**: 기기에서 안 돌렸다. `./test/htp/bisect_moe_v1.sh` 한 번이면
답이 나온다.

### 33.2 bisect 계획 (33.2a가 틀렸을 때만)
