...
| **block-level calls: the dense FFN and the conv block as one call each** | `51_block_calls_on_htp.md` — dense FFN through the MoE kernel (measured 10.4 ms/call, as predicted), and the conv block kernel `hexkl_conv_block.c` (weights resident, rows streamed twice; measured 4.9 ms/call as predicted, prefill 848 → 732, perplexity: the conv block costs nothing, the dense FFN costs 4%, so dense stays off). The `conv_block_engine` switch and the D measurement config |
