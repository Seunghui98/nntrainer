...
| **int32 → u8 requantization straight from the accumulator (arXiv 2511.11248), a separate session** | `53_int_requant_task.md` — where the f32 hops are in the MoE, FC and conv epilogues, what is already measured (the epilogue is hidden under the HMX since 51 §2.22: 0.35 ms of 14.45 exposed, so the win can only be structural — the row-scan barrier, VTCM, or an int8 acc_read if HexKL has one), the accuracy table by scale granularity, the blanks to fill from the paper, the gated order and the first prompt |
