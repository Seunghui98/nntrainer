...
| **making the MoE FFN fast — CURRENT** | `43_moe_ffn_measured_next_levers.md` — the E2E path runs and is measurably NOT faster than CPU; per-stage device numbers, the ranked levers, and the one-lever-at-a-time protocol. **Supersedes 41 §5's ordering.** Start here for perf work |
