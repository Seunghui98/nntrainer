...
### 9.3 `ion_total_gb`가 없다 — 이것이 판정 숫자다

3단계가 최대 40×256 MB = 10 GB를 할당하려 했고 출력이 그 직전에 끊겼다. **OOM으로 프로세스가 죽은 것으로 보인다.** 수정: 상한을 6 GB로 낮추고 **청크마다 즉시 출력**해서 중간에 죽어도 마지막 유효값이 로그에 남게 했다.

### 9.4 지금까지의 판정

| | |
|---|---|
| DSP 힙 | **3.75 GB** — 필요한 4.10의 91% |
| 등록 경로 | 1.89 GB — **재설계 대상이지 하드웨어 한계가 아님** |
| ION 총량 | **미측정 — 다음 실행** |

힙만으로도 3.75 GB이므로 **필요량의 91%가 이미 닿는다.** 남은 9%는 (a) ION이 힙과 별도 공간이면 자동 해결, (b) 아니면 MoE 22 레이어 중 20개만 HTP (3.53 GB)로도 계획의 대부분이 성립한다. §8.5의 "<2 GB면 상주 불가" 분기는 **사라졌다.**
