...
### 21.1 결과 (2026-09-16, 기기) — **손해, 기본 OFF**

| us/콜 | O2 (§19.2) | O1 | |
|---|---:|---:|---|
| blocks | 1004 (45.6/콜) | **926 (42.1)** | 꼬리 **3.5개/콜**만 ≤16행. §14의 "13.6"은 모든 둘째 블록이었고 대부분 16행 초과 |
| mm + acc_read | 8900 + 2985 | 8531 + 2764 | **−0.59** — 3.5 × 252 = 0.89의 2/3 |
| scatter | 23 | **471** | 꼬리가 expert 차례에 안 끝나 있음: +0.45 |
| requant | 1248 | **1727** | 동기 run이 꼬리 유닛 중인 워커를 기다림: +0.48 |
| dequant | 1178 | 1324 | 같은 간섭: +0.15 |
| dsp / host | 15898 / 18258 | 16363 / 18568 | **+0.47 / +0.31** |
| 비프로파일 prefill | 924 | 913, 954 | 차이 없음(잡음 ±20) |
| 텍스트 | | 동일 | 비트동일 경로는 맞다 |

**왜 안 됐나.** ① 대상이 적다: ≤16행 꼬리는 콜당 3.5개라 최대 이득이 0.9 ms다. ② 풀의
포그라운드 잡(에필로그 submit, requant run)은 워커가 진행 중인 bg 유닛을 마치길 기다린다 —
유닛 하나 ≈20–40 us × 콜당 ~140회 동기점 = 간섭 0.6 ms. 이건 이 풀 설계에 내재한다(유닛
안에서 선점 불가). ③ 꼬리가 expert 차례보다 늦다(bg가 받는 시간은 워커 유휴의 ~60%).
임계를 올리면 ①은 좋아지지만 ②③이 유닛 크기에 비례해 나빠진다. **기본 OFF**
(`MOE_TAIL_MAX_ROWS 0`), 코드와 호스트 검사(-D로 16)는 유지. `swiglu` 열의 꼬리 워커 시간
계측은 이 실행의 skel에 없었다(0.0) — 켤 일이 있으면 그것부터.
...
### 22.1 결과 (2026-09-16, 기기) — O4 무효·되돌림, gather·워밍업 유지, 다음은 **워커 spin**

PROFILE=2, `M>1` 행 calls=23 (워밍업 콜 1 포함, blocks 1036 = 1004 + 32). 블록당으로 환산:

| us/블록 | O2 (§19.2) | §22 빌드 | |
|---|---:|---:|---|
| requant | 27.3 | 28.2 | **O4가 안 먹었다**: 스캔이 아니라 pack의 pool_run 비용이었다 |
| dequant | 25.8 | 27.5 | 극값 추적이 에필로그를 무겁게 함 |
| gather | 5.0 | 3.0 | ✓ 다음 블록 act 조기 큐잉 |
| rest | 249/콜 | 392/콜 | main의 fold 비용 |
| alloc | 109/콜 | 52/콜 | 워밍업이 첫 성장 흡수 |
| transport | 2360/콜 | 2060/콜 | 첫 접촉 몫 감소로 추정 |

비프로파일 3회: **898 / 954 / 915** (최소 898 = 494 TPS, 최고 기록), decode 20.2 셋 다.
텍스트 동일.

**O4 되돌림**(코드 삭제). 근거: requant 열의 정체는 행 스캔이 아니라 **워커 깨우기**다 — 스캔의
pool_run을 없애자 그 wake 비용이 pack의 pool_run으로 옮겨갔을 뿐이다. 워커는 잡이 없으면
futex로 자고, 다음 submit/run이 깨우는 데 수 us × 3 워커가 든다. 콜당 ~140회 동기점 →
≈1 ms. DEQUANT 25.8 us/블록도 일(≈18 us)보다 크게 읽히는 이유가 같다.

**다음(커밋됨, 미측정): 워커 spin-before-sleep.** `hvx_worker_pool_thread_entry`가 잠들기 전
≈100 us(pause(#255) × 400) seqn을 감시한다. 콜 안에서는 47 us마다 submit이 오므로 워커가
잠들지 않고, 콜 사이엔 100 us 뒤 잠든다. 기대: dequant·requant에서 합쳐 −0.5~−1.0 ms/콜.
위험: spin이 main의 HMX issue 스레드와 SMT 슬롯을 나눠 `mm`이 오를 수 있다 — 그 열로 판정.

### 22.2 결과 (2026-09-16, 기기) — 워커 spin **효과**, prefill 848 ms / **523 TPS**

| us/콜 (M>1, calls=23) | §22.1 | spin | |
|---|---:|---:|---|
| host | 18043 | **17498** | −0.55 ms/콜 → −12 ms |
| dequant | 1237 | **849** | 깨우기 비용이 빠짐 |
| requant | 1271 | **1059** | 같음 |
| mm | 9022 | 9252 | **+0.23**: spin 스레드가 HMX issue와 SMT 슬롯을 나눈다(예상한 부작용, 이득의 40%) |
| rest | 392 | 192 | |
| decode host (M==1) | 1554 | 1566 | dequant −9 us, mm +26 → 중립 |

비프로파일: **848 ms (523.6 TPS)**, decode 19.7. 텍스트 동일.

**콜 17.5 ms의 구성:** HMX 12.2 (mm 9.25 + acc 2.93, 구조) + transport 2.1 (바이트) = 14.3 (82%).
나머지 3.2 = requant 1.06 + dequant 0.85 + stage 0.36 + quant 0.26 + drain 0.22 + rest 0.19 +
gather 0.14 + scatter 0.05 + push 0.05. 전부 숨겨도 −2 ms/콜(−44 ms)이고 각각은 ≤1%짜리다.
spin 길이(400 → 200)로 mm의 +0.23을 일부 되찾을 수는 있다 — 노브, 미측정.

**FFN 범위 안의 prefill 최적화는 여기까지다.** 181 → 523 TPS. 남은 큰 것은 §20.1의 10번(청크
파이프라인)뿐이고, 그건 커널 밖이다.
