...
| Method | Result | Note |
| :--- | :--- | :--- |
| Transient probe (`PoolProbe_MeasureMaxResidentBytes`): 4 MB chunks, all freed before measuring | **4000 MB** | not usable for pin-once sizing |
| Sustained-pin probe (`PoolProbe_MeasureMaxSustainedPinBytes`): 4/6/6 MB cycling, 84 allocations, never freed | **448 MB** | hit the loop cap (84), allocator never failed |
...
| `NNTR_HTP_VERIFY_PREBAKED_WH=1` | on every pre-baked hit, recomputes the transient conversion and memcmps it against the registered bytes; logs per-weight OK/MISMATCH to stderr |
| `NNTR_HTP_VERIFY_PREFILL_MM=1` | diffs live NPU output against the CPU reference on real inputs (always-on diagnostic) |
