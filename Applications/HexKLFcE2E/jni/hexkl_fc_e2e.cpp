...
WeightQuant quantWeightNK(const std::vector<float> &W_nk, int N, int K) {
  ...
  for (int n = 0; n < N; ++n) {
    ...
    for (int k = 0; k < K; ++k)
      max_abs =
        std::max(max_abs, std::fabs(W_nk[static_cast<size_t>(n) * K + k]));
  ...
  for (auto &node : graph) {
    ...
    if (verbose) {
      ...
      for (unsigned int w = 0; w < nw; ++w)
        std::printf(" [%u]=%s", w,
                    node->getRunContext().getWeightName(w).c_str());
      std::printf("\n");
    }
    if (nw == 0)
      continue;
...
int main(int argc, char **argv) {
  ...
  if (!via_model) {
    ...
    out_fp32 = forwardDirect(a.M, a.N, a.K, A, makeFp32Weight(a.K, a.N, W_kn),
                             bias, false);
  }
