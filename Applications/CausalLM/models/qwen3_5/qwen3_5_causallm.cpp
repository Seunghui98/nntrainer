...
Tensor Qwen3_5Transformer::createMlp(const int layer_id, int dim,
  ...
  // Layer creation order matches support_qwen3_5_1 branch: gate, up, swiglu, down
  // Topological sort will reorder to match weight_converter save order.
...
Tensor Qwen3_5Transformer::createAttention(const int layer_id, int seq_len,
  ...
  // Layer creation order matches support_qwen3_5_1 branch exactly:
  // V, K, K_norm, Q, Q_gate, Q_norm, attention, attn_gate, O
  // Topological sort will reorder to match weight_converter save order.
