...
const std::map<std::string, DataType> dtype_str_map = {
  ...
  {"Q4_K", DataType::Q4_K}, {"Q8_0", DataType::Q8_0},
...
std::string generateOutputBinName(const std::string &original_bin,
  ...
  std::vector<std::string> dtype_suffixes = {
    "_fp32", "_fp16", "_q40",   "_q4_0", "_q6k",  "_q6_k",
    "_q4k",  "_q4_k", "_qs4cx", "_q80",  "_q8_0"};
 ...
 * lookup tables. The embedding_layer supports Q4_0 and Q6_K lookup/save (Q4_0
 * needs width % 32 == 0 — BD_DIM = 256, OK), so embd_dtype is fully
 * configurable (FP32 / Q4_0 / Q6_K) via --embd_dtype.
 *
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // The BERT decoder nests its layer count under cfg["decoder"] when the
    // combined config.json is reused; fall back to the top-level field for a
    // flat decoder config.
    ...
    if (is_decoder) {
      ...
      // (Q4_0 or Q6_K), LayerNorms/tied-LM-head stay FP32.
      layer_dtype_map =
