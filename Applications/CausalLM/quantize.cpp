 ...
 *   - layer{i}_wq_gate           : Qwen3.5 self-attention gate projection
 *   - layer{i}_attention_out     : attention output projection (FC layer)
 *   - layer{i}_ffn_up/gate/down  : FFN layers (FC layers)
 *   - layer{i}_gdn_in_proj_qkv   : Qwen3.5 GDN input projection (FC layer)
 *   - layer{i}_gdn_in_proj_a/b/z : Qwen3.5 GDN projections (FC layers)
 *                                   Note: a/b have width=num_v_heads which may
 *                                   not be divisible by 32 (Q4_0 block size),
 *                                   so they are excluded from quantization.
 *   - layer{i}_gdn_out_proj      : Qwen3.5 GDN output projection (FC layer)
 *   - layer{i}_attention_norm, layer{i}_ffn_norm : RMSNorm layers
 *   - output_norm                : final RMSNorm
 *   - output_of_causallm         : LM head (FC layer)
 *
 * For hybrid architectures like Qwen3.5, is_self_attn_layer[i] selects
 * between self-attention and linear-attention (GDN) layer naming.
 */
...
buildLayerDtypeMap(int num_layers, DataType fc_dtype, DataType embd_dtype,
  ...
  for (int i = 0; i < num_layers; ++i) {
    ...
    if (fc_dtype != DataType::FP32 && fc_dtype != DataType::NONE) {
      // For hybrid models (Qwen3.5), decide per-layer block type
      bool is_self_attn = (i < (int)is_self_attn_layer.size())
                            ? is_self_attn_layer[i]
                            : true; // default: all layers are self-attention

      if (is_self_attn) {
        // Self-attention FC layers
        ...
        // Qwen3.5 self-attention has an extra gate projection
        dtype_map[prefix + "_wq_gate"] = fc_dtype;
      } else {
        // Qwen3.5 linear attention (GDN) decomposed FC layers
        dtype_map[prefix + "_gdn_in_proj_qkv"] = fc_dtype;
        // NOTE: in_proj_a/b have width=num_v_heads(16), not divisible by 32
        // (Q4_0 block size). Skip quantization for these.
        dtype_map[prefix + "_gdn_in_proj_z"] = fc_dtype;
        dtype_map[prefix + "_gdn_out_proj"] = fc_dtype;
      }

      // FFN FC layers (shared by both self-attn and linear-attn blocks)
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // Handle VLM nested config (e.g. Qwen3.5 has cfg["text_config"])
    ...
    bool tie_word_embeddings = text_cfg.contains("tie_word_embeddings")
                                 ? text_cfg["tie_word_embeddings"].get<bool>()
                                 : false;

    // Determine self-attention vs linear attention layer pattern for hybrid
    // models (Qwen3.5). Empty vector means "all self-attention" (default).
    std::vector<bool> is_self_attn_layer;
    if (nntr_cfg.contains("self_attn_layers")) {
      is_self_attn_layer.assign(num_layers, false);
      for (auto idx :
           nntr_cfg["self_attn_layers"].get<std::vector<unsigned int>>()) {
        if ((int)idx < num_layers)
      ...
      is_self_attn_layer.assign(num_layers, false);
      for (unsigned int i = pattern - 1; i < (unsigned int)num_layers;
           i += pattern)
    ...
    auto layer_dtype_map =
      buildLayerDtypeMap(num_layers, fc_dtype, embd_dtype, lmhead_dtype,
                         tie_word_embeddings, is_self_attn_layer);
