 ...
 *
 * When tie_word_embeddings is true, output_of_causallm shares its weight
 * with embedding0 and its save() is a no-op — including it in the map is
 * harmless but misleading, and the serialized nntr_config should advertise
 * a single shared dtype for the two layers. We omit it here and let the
 * caller copy embd_dtype into lmhead_dtype in the rewritten nntr_config.
 */
...
buildLayerDtypeMap(int num_layers, DataType fc_dtype, DataType embd_dtype,
  ...
  // shares the embedding weight and TieWordEmbedding::save() is a no-op
  // for the lm_head mode anyway).
 ...
 *
 * The exception is thrown deep in NeuralNetwork::save, so without an
 * up-front check we either burn time loading the whole model just to
 * throw, or trip a corrupt save path. Validate here and report the
 * offending mismatch before any work happens.
 *
 * Tied embeddings get an extra restriction: TieWordEmbedding::save only
 * knows how to serialize Q6_K (and passes through FP32). The lm_head side
 * shares the embedding weight and is a save no-op, so the embedding dtype
 * is what gets written for both. Q4_0 / Q4_K requests on a tied model are
 * therefore rejected with guidance to use Q6_K.
 */
void validateRequestedDtypes(const json &nntr_cfg, DataType fc_dtype,
  ...
  // verbatim weight.save(). The only path that actually re-encodes a
  // weight requires the source to be FP32, so reject (src=quantized,
  // dst=different quantized). dst=FP32 against a quantized source is not
  // a true re-quantization either (no dequantizer in save), but the layer
  // treats it as passthrough, so leave it alone here.
  ...
  if (tie_word_embeddings) {
    if (embd_dtype != lmhead_dtype) {
      throw std::runtime_error(
        ...
        ") differ. With tied embeddings the two layers share one weight "
        "and must use the same dtype.");
    }
    auto reject_tied = [](DataType dt, const char *opt) {
      if (dt == DataType::Q4_0 || dt == DataType::Q4_K) {
        throw std::runtime_error(
          ...
          ". TieWordEmbedding only supports Q6_K (or FP32) for "
          "quantization because the shared weight has to serve both the "
          "embedding lookup and the lm_head matmul. Re-run with "
          "--embd_dtype Q6_K --lmhead_dtype Q6_K.");
      }
    };
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // models or BERT-style encoders that do not declare it). Default to
    // false so existing flows keep their current behaviour.
    ...
    // Validate the requested combination *before* paying the cost of
    // loading the model. The per-layer save path can only re-quantize FP32
    // sources, and tied embeddings have no Q4_0/Q4_K serializer — without
    // an up-front check we would either burn a multi-GB load just to
    // throw deep in save(), or hit a partially-corrupt output file.
    ...
    // Resolve the dtype each bucket was actually written with: layer
    // save() passes the weight through verbatim when the requested target
    // matches the in-memory dtype (e.g. when a Q6_K source is "quantized
    // to FP32"), so the output .bin keeps the source's dtype rather than
    // the option value. Reflect that in the rewritten nntr_config so the
    // next run creates layers with the right weight type.
