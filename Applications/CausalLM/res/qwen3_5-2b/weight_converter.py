...
def save_qwen3_5_for_nntrainer(params, config, dtype, file):
    ...
    def save_self_attn(layer_prefix):
        ...
        Must match Qwen3_5Transformer::createAttention() layer creation order:
          V, K, K_norm, Q(query), Q_gate, Q_norm, [mha_core-no wt], [attn_gate-no wt], O
        """
        prefix = f"{layer_prefix}self_attn."

        # Split q_proj into query (first half) and gate (second half)
        ...
        # 3) K norm (reshaped_rms_norm: +1.0 offset)
