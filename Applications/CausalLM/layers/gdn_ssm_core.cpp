 ...
 * @date   9 April 2026
 * @brief  GDN SSM Core layer implementation
 */
...
void GdnSsmCoreLayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  // State: recurrent_state (batch, 1, num_v_heads * head_k_dim, head_v_dim)
...
void GdnSsmCoreLayer::incremental_forwarding(
  ...
  for (unsigned int b = 0; b < batch_size; ++b) {
    ...
    for (unsigned int t = 0; t < seq_len; ++t) {
      ...
      // Split conv_out into Q, K, V (copy to mutable buffer)
      ...
      // SSM state update + attention output per head
      for (unsigned int h = 0; h < num_v_heads; ++h) {
      ...
      // Gated RMS norm: rms_norm(o) * silu(z)
      for (unsigned int h = 0; h < num_v_heads; ++h) {
        float *o_h = y + h * head_v_dim;
        std::vector<float> normed(head_v_dim);
        rms_norm(o_h, normed.data(), norm_w, head_v_dim);
        for (unsigned int vi = 0; vi < head_v_dim; ++vi)
          o_h[vi] = normed[vi] * silu(z_t[h * head_v_dim + vi]);
      }
    }
  }
}
...
void GdnSsmCoreLayer::updateTensorsByInputDimensions(
  ...
  // a, b inputs update with their own dimensions
