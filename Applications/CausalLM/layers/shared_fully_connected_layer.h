 ...
 * @brief  Shared Fully Connected Layer Class (No file read)
...
public:
  ...
  /**
   * @brief     Finalize creating the layer node (Independent Mode)
   * @param     context InitLayerContext
   */
  void finalize_independent(nntrainer::InitLayerContext &context);

  /**
   * @brief     Finalize creating the layer node (Shared Mode)
   * @param     context InitLayerContext
   */
  void finalize_shared(nntrainer::InitLayerContext &context);
  ...
  /**
   * @brief     Incremental forward Propagation of a layer (Independent Mode)
   * @param     from start step
   * @param     to end step
   * @param     training true if training, false if inference
   */
  void incremental_forwarding_independent(nntrainer::RunLayerContext &context,
                                          unsigned int from, unsigned int to,
                                          bool training);

  /**
   * @brief     Incremental forward Propagation of a layer (Shared Mode)
   * @param     from start step
   * @param     to end step
   * @param     training true if training, false if inference
   */
  void incremental_forwarding_shared(nntrainer::RunLayerContext &context,
                                     unsigned int from, unsigned int to,
                                     bool training);
   ...
   */
  const std::string getType() const override { return SharedFullyConnectedLayer::type; };
...
private:
  std::tuple<nntrainer::props::Unit, nntrainer::props::DisableBias,
             ...
             nntrainer::props::BiasInitializer,
             props::SharedMode,
