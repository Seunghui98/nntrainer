...
static int runDumpEncoder(int argc, char *argv[]) {
  ...
  try {
    json cfg = quick_ai::LoadJsonFile(dir + "/config.json");
    json gen_cfg = json::object();
    json nntr_cfg = quick_ai::LoadJsonFile(dir + "/nntr_config.json");
    ...
    auto enc =
      std::make_unique<quick_ai::Siglip2VisionEncoder>(cfg, gen_cfg, nntr_cfg);
    enc->initialize();
    ...
    const size_t dim = quick_ai::Siglip2VisionEncoder::ENC_TO_DEC_DIM;
...
static int runDecoderInitParity(const std::string &model_dir) {
  ...
  try {
    json nntr = quick_ai::LoadJsonFile(model_dir + "/nntr_config.json");
  ...
  try {
    ...
    auto dec = std::make_unique<quick_ai::BertDecoder>();
