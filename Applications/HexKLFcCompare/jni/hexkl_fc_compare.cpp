...
// Per-output-channel symmetric weight quant (mirrors
// quantize_qint{4,8}_weight): scale[n] = max_abs(W[n,:]) / q_max, w_q[n,k] =
// clamp(round(W/scale), ±q_max), zp_corr[n] = 128 * sum_k w_q[n,k]. w_q is
// logical [N,K] (w_q[n*K+k]).
struct WQuant {
...
static std::vector<float> dequant(const int32_t *c_i32, int M, int N,
  ...
  for (int m = 0; m < M; ++m)
    for (int n = 0; n < N; ++n)
      c[(size_t)m * N + n] =
        act_scale * sc[n] * ((float)c_i32[(size_t)m * N + n] - (float)zp[n]);
  return c;
}
...
static bool runViaNntrainer(int M, int N, int K, int bits,
  ...
  // Doing this first also keeps the backend bring-up out of the cold-run
  // figure.
  {
...
int main(int argc, char **argv) {
  ...
  if (via_nntr) {
    ...
    if (!npu_ok)
      std::fprintf(stderr,
                   "[warn] sdkl_npu_initialize failed (err=%d); "
  ...
  if (a.sweep) {
    ...
    if (do8)
      std::printf(" u8i8 exec us |");
    if (do4)
      std::printf(" u8i4 exec us |");
    std::printf("\n|---|---|---|");
    if (do8)
      std::printf("---|");
    if (do4)
    ...
    for (const auto &s : shapes) {
      ...
      if (do8) {
        QResult q8 = runQuant(s.M, s.N, s.K, As, Ws, 8, a.warmup, a.iters,
                              npu_ok, domain, via_nntr);
        std::printf(" %12.1f |", q8.t.kernel_us);
        if (base8 < 0.0)
          base8 = q8.t.kernel_us;
        full8 = q8.t.kernel_us;
      }
      if (do4) {
        QResult q4 = runQuant(s.M, s.N, s.K, As, Ws, 4, a.warmup, a.iters,
                              npu_ok, domain, via_nntr);
        std::printf(" %12.1f |", q4.t.kernel_us);
        if (base4 < 0.0)
          base4 = q4.t.kernel_us;
    ...
    if (do8)
      std::printf("  u8i8=%.1f us", base8);
    if (do4)
      std::printf("  u8i4=%.1f us", base4);
    std::printf("\ncompute-only at the largest shape       :");
    if (do8)
      std::printf("  u8i8=%.1f us", full8 - base8);
    if (do4)
      std::printf("  u8i4=%.1f us", full4 - base4);
  ...
  auto dumpMethod = [&](const char *tag, float err, const QResult &r) {
    ...
    std::printf("    H2D activation upload                %10.1f\n",
                r.t.h2d_act_us);
    std::printf(
      "    Steady NetRun (mean execute)         %10.1f   <- compute\n",
      r.t.kernel_us);
    std::printf("      min / median / max                 %10.1f /%8.1f /"
                "%8.1f\n",
                r.t.kernel_min_us, r.t.kernel_med_us, r.t.kernel_max_us);
