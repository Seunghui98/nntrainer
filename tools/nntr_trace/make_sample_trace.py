...
Hexagon cDSP, one track per engine (main thread, ThreadManager workers, FastRPC seam,
DSP caller thread, HMX, HVX worker pool, DMA), flow arrows from every host
FastRPC call to the DSP work it triggered, and counter tracks for VTCM and
HMX occupancy.
...
Usage
    ...
    tools/nntr_trace/make_sample_trace.py -o out.json --layers 4
...
def hvx_split(tr, name, cat, ts, total_us, n_units, layer, elems, **extra):
  ...
  thread, the rest on parked workers, with a small fork/join tax.
  """
  ...
  for i in range(n):
    ...
    tr.x(PID_DSP, TID_HVX[i], name, cat, ts + fork, d, layer=layer,
         engine="HVX", unit=i, units=n, elems=int(elems / n), **extra)
...
def dsp_fc_call(tr, ts, layer, opname, M, K, N, weight_us, pipelined,
  ...
  ts = hvx_split(tr, "quant f32->u8 AH", "dsp.hvx", ts, quant_us, K // 32,
                 layer, M * K, op=opname, stage="quant")
  n_tiles = (M // 64) * (N // 32)
  ...
  tr.x(PID_DSP, TID_DSP_MAIN, "layer_run " + opname, "dsp.call", t0, end - t0,
       layer=layer, op=opname, M=M, K=K, N=N, pipelined=pipelined)
  tr.counter(PID_DSP, "VTCM (KB)", end, used=256)
  return end


def fastrpc(tr, ts, layer, name, in_bytes, out_bytes, transport_us, body):
  ...
  tr.x(PID_HOST, TID_MAIN, "wait " + name, "host.wait", ts, t_ret - ts,
       layer=layer, engine="HTP")
  return t_ret


def cpu_omp(tr, ts, layer, name, total_us, elems):
...
def prefill_layer(tr, ts, li, seq, pipelined):
  layer = "layer%d" % li
  t_layer = ts
  ...
  def sdpa(t):
    ...
    for h in range(2):  # two of 16 heads drawn out, the rest folded below
      ...
      t = hvx_split(tr, "softmax (band)", "dsp.hvx", qk_end, 150, 8, layer,
                    seq * 256, op="sdpa", head=h, stage="softmax")
      for j in range(4):
        t = hvx_split(tr, "P quant", "dsp.hvx", t, 20, 4, layer, 64 * 256,
                      op="sdpa", head=h, block=j, stage="quant")
    ...
    tr.x(PID_DSP, TID_HMX, "Q.Kt / P.V (14 heads folded)", "dsp.hmx", t,
         rest * 0.24, layer=layer, op="sdpa", engine="HMX", elems=14 * seq * seq * 128 * 2)
...
def decode_layer(tr, ts, li):
  layer = "layer%d" % li
  t_layer = ts

  def fc(t, op, K, N):
    t0 = t
    t = hvx_split(tr, "quant f32->u8 AH", "dsp.hvx", t, 8, 2, layer, K, op=op)
    ...
    tr.x(PID_DSP, TID_DSP_MAIN, "layer_run " + op, "dsp.call", t0, t - t0,
         layer=layer, op=op, M=1, K=K, N=N)
    return t

  ts = tr.x(PID_HOST, TID_MAIN, "rmsnorm", "host.cpu", ts, 6, layer=layer,
            engine="CPU")
  ts = fastrpc(tr, ts, layer, "mm_u8i8_layer[q,k,v]", 4096, 16384,
               FASTRPC_M1_US, lambda t: fc(t, "qkv_proj", 1024, 4096))
  ts = tr.x(PID_HOST, TID_MAIN, "rope + kv_append", "host.cpu", ts, 14,
            layer=layer, engine="CPU")

  def attn(t):
    t0 = t
    t = hvx_split(tr, "q quant", "dsp.hvx", t, 6, 2, layer, 2048, op="sdpa")
    d = tr.x(PID_DSP, TID_DMA, "Kt_wh blocks", "dsp.dma", t, 25, layer=layer,
             op="sdpa")
    t = tr.x(PID_DSP, TID_HMX, "Q.Kt (M=2 pad 64)", "dsp.hmx", d, 30,
             layer=layer, op="sdpa", engine="HMX", elems=16 * 1024 * 128)
    t = tr.x(PID_DSP, TID_HVX[0], "acc_copy + dequant", "dsp.hvx", t, 50,
             layer=layer, op="sdpa", engine="HVX", elems=16 * 1024)
    t = tr.x(PID_DSP, TID_HVX[0], "softmax", "dsp.hvx", t, 22, layer=layer,
             op="sdpa", engine="HVX", elems=16 * 1024)
    d = tr.x(PID_DSP, TID_DMA, "V_wh blocks", "dsp.dma", t, 25, layer=layer,
             op="sdpa")
    t = tr.x(PID_DSP, TID_HMX, "P.V (M=2 pad 64)", "dsp.hmx", d, 30,
             layer=layer, op="sdpa", engine="HMX", elems=16 * 1024 * 128)
    t = tr.x(PID_DSP, TID_HVX[0], "acc_copy + dequant", "dsp.hvx", t, 50,
             layer=layer, op="sdpa", engine="HVX", elems=16 * 128)
    tr.x(PID_DSP, TID_DSP_MAIN, "attn_forward", "dsp.call", t0, t - t0,
         layer=layer, op="sdpa", n_query=1, kv=1024)
    return t
  ts = fastrpc(tr, ts, layer, "attn_forward", 8192, 8192, FASTRPC_M1_US, attn)
  ts = fastrpc(tr, ts, layer, "mm_u8i8_layer[o]", 8192, 4096, FASTRPC_M1_US,
               lambda t: fc(t, "o_proj", 2048, 1024))
  ts = tr.x(PID_HOST, TID_MAIN, "residual + rmsnorm", "host.cpu", ts, 9,
            layer=layer, engine="CPU")
  ts = fastrpc(tr, ts, layer, "mm_u8i8_layer[gate,up]", 4096, 24576,
               FASTRPC_M1_US, lambda t: fc(t, "gate_up_proj", 1024, 6144))
  ts = tr.x(PID_HOST, TID_MAIN, "silu * up", "host.cpu", ts, 5, layer=layer,
            engine="CPU")
  ts = fastrpc(tr, ts, layer, "mm_u8i8_layer[down]", 12288, 4096,
               FASTRPC_M1_US, lambda t: fc(t, "down_proj", 3072, 1024))
...
def build(n_layers, seq, pipelined):
  tr = Trace()
  ...
  for li in range(n_layers):
    ts = prefill_layer(tr, ts, li, seq, pipelined)
  ...
  for tok in range(2):
    ...
    tr.x(PID_HOST, TID_MAIN, "decode token %d" % (tok + 1), "host.phase", t_tok,
         ts - t_tok, phase="decode", token=tok + 1)
    ts += 30
  return {
    "displayTimeUnit": "ms",
    "traceEvents": tr.ev,
...
def main():
  ...
  a = ap.parse_args()
  out = a.out or ("trace_pipelined.json" if a.pipelined else "trace_asbuilt.json")
  doc = build(a.layers, a.seq, a.pipelined)
