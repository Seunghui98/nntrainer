 ...
 *        (M>1).
 *
 * @note Prefill touches each weight once, so an evicting cache smaller than
 * the working set thrashes to ~0% hits on a sequential scan. Weights that
 * would exceed PREFILL_WH_PIN_MAX_BYTES are not cached and fall back to the
 * transient conversion path instead -- correct, just slower.
 */
...
/**
 * @brief Host-RAM registry of pre-baked (offline-converted) WH weight bytes,
 *        keyed by the RM host weight pointer.
 *
 * @note These live in normal host RAM rather than the NPU DMA pool, so the
 * whole model can stay resident without touching the ~48 MB pin budget;
 * shgemm memcpies them into NPU scratch per call.
 */
...
void shgemm_f32f16_f32(const unsigned int TStorageOrder, bool TransA,
  ...
  // --- NPU-accessible staging buffers ---
  // Borrowed from the g_scratch_* pools, which own them across calls, so no
  // error path here frees anything.
