  ...
  case ml::train::ModelFormat::MODEL_FORMAT_SAFETENSORS: {
    ...
    for (auto iter = model_graph.cbegin(); iter != model_graph.cend(); iter++) {
      ...
      for (unsigned int i = 0; i < weights.size(); ++i) {
        ...
        // Determine dtype string for safetensors header
        std::string dtype_str;
        switch (dtype) {
        ...
        // Build shape from TensorDim (skip leading 1-dims for compact shape)
        auto dim = w.getDim();
        std::vector<size_t> shape;
...
void NeuralNetwork::load(const std::string &file_path,
  ...
  if (format == ml::train::ModelFormat::MODEL_FORMAT_BIN) {
    ...
    probe.close();
  } else if (format == ml::train::ModelFormat::MODEL_FORMAT_SAFETENSORS) {
    /**
     * Parse safetensors header:
     *   [8B]  header_size (little-endian uint64)
     *   [header_size B] JSON: {"name": {"dtype":..,"shape":..,"data_offsets":[start,end]}, ...}
     *   [data section]
     */
    auto probe = checkedOpenStream<std::ifstream>(
      f_path, std::ios::in | std::ios::binary);
    ...
    // Parse JSON header to extract tensor name -> (offset, size) mapping
    // Format: {"__metadata__":{...}, "name":{"dtype":"F32","shape":[...],"data_offsets":[start,end]}, ...}
    auto parse_safetensors_header =
      [](const std::string &json,
        ...
        // Skip to find data_offsets within a value object
        auto skip_value = [&]() {
          skip_ws();
          if (pos >= json.size())
            return;
   ...
   *   Named format (v1/safetensors): look up by weight name → order-independent.
  ...
  for (auto iter = model_graph.cbegin(); iter != model_graph.cend(); iter++) {
    ...
    for (unsigned int wi = 0; wi < weights.size(); ++wi) {
      ...
      if (is_named_format) {
        // Named format (v1 BIN or safetensors): name-based offset lookup
        ...
        if (it != name_offset_map.end()) {
          ...
          ml_logw("Weight '%s' not found in header, using sequential offset",
  ...
  case ml::train::ModelFormat::MODEL_FORMAT_SAFETENSORS: {
    ...
    // Safetensors loading uses the same parallel mmap strategy as BIN v1.
    // File offsets have already been assigned via name-based lookup above.
    if (exec_mode == ml::train::ExecutionMode::INFERENCE) {
      std::vector<std::thread> threads;
      threads.reserve(model_graph.size());
      for (auto iter = model_graph.cbegin(); iter != model_graph.cend();
