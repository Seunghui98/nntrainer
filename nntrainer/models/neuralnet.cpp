...
void NeuralNetwork::load(const std::string &file_path,
  ...
  case ml::train::ModelFormat::MODEL_FORMAT_BIN: {
    ...
    // scheduled above. The inference path dispatches one thread per graph
    // node and reads from `weight_file_offset .. + weight_bytes` via either
    // ifstream::read or mmap+memcpy. mmap silently succeeds for offsets
    // past EOF, and the subsequent memcpy then SIGSEGVs (the user-visible
    // symptom is a bare "segmentation fault" partway through Tensor::read
    // on whichever node happened to draw the last shard). Catching the
    // size mismatch here surfaces the real problem — the .bin does not
    // match the model the graph expects — instead of a cryptic crash
      ...
      if (!ec && on_disk < start_from) {
        ...
        msg << "model weight file is smaller than the graph expects: "
            ...
            << " bytes for its weights. The most common cause is a config "
               ...
               "do not agree with what the .bin was actually written with, "
