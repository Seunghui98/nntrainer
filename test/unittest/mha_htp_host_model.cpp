 ...
 * @bug    No known bugs.
 *
 * See mha_htp_host_model.h for the contract and for the five load-bearing
...
MhaSoftmaxOut mha_softmax_blocked_ref(const std::vector<const float *> &seg,
  ...
  for (uint32_t m = 0; m < M; ++m) {
     ...
     * naturally (MHA_HTP_U8_TASKS.md §1.7). The sink is a single scalar exp
...
void mha_htp_host_forward(uint32_t n_query, uint32_t kv_from, uint32_t nch,
                          ...
                          float *out) {
  assert(M_band <= T); /* property 5, MHA_HTP_PLAN.md §5.4 */
    ...
    for (uint32_t b0 = 0; b0 < M; b0 += M_band) {
      ...
      for (uint32_t j = 0; j < n_blocks; ++j) {
        ...
        mm_u8_iX_dequant(p_u.data(), p_scale.data(), p_zp.data(),
                         v_rm[j].data(), v_scale[j].data(), v_cs[j].data(), mb,
