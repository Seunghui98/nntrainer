 ...
 * while dropping K to i4 costs ~3x, which INVERTS MHA_HTP_U8_TASKS.md §0.2's
...
void ground_truth(const Cfg &c, uint32_t kv_from, const std::vector<float> &q,
  ...
  for (uint32_t i = 0; i < c.n_query; ++i) {
    ...
    compute_kcaches_fp32_reference(
      q.data() + (size_t)i * nHq * c.head_dim, k32.data(), scores.data(),
      (int)end, (int)c.nch, (int)c.head_dim, (int)c.gqa, lws);
