 ...
 *
 * @file   sdkl_npu_probe.cpp
 * @brief  Minimal probe to test raw sdkl NPU calls
...
int main() {
  ...
  // ----------------------------------------------------------------
  {
    ...
    for (int n = 0; n < N && n < K; ++n)
      W_f32[n * K + n] = 1.f; // diag=1

    std::vector<_Float16> W_f16(N * K);
    for (int i = 0; i < N * K; ++i)
      W_f16[i] = (_Float16)W_f32[i];
    ...
    if (err) {
      fprintf(stderr, "FAIL case1: rm_to_wh err=%d\n", err);
      ++failures;
      goto case2;
    }

    memset(Ab, 0, M * N * sizeof(float));
    err = sdkl_npu_mm_f32f16_f32(domain, M, N, K, static_cast<float *>(Ab),
                                 static_cast<const float *>(Xb),
                                 static_cast<const _Float16 *>(Wb));
    if (err) {
      fprintf(stderr, "FAIL case1: npu_mm err=%d\n", err);
      ++failures;
      goto case2;
    }
...
case2:
  ...
  // ----------------------------------------------------------------
  {
    ...
    err = sdkl_npu_mm_f32f16_f32(domain, M, N, K, static_cast<float *>(Ab),
                                 ...
                                 static_cast<const _Float16 *>(Wb));
    if (err) {
      fprintf(stderr, "FAIL case2: npu_mm err=%d\n", err);
      ++failures;
      goto case3;
    }
    memcpy(C_npu.data(), Ab, M * N * sizeof(float));
...
case3:
  ...
  // ----------------------------------------------------------------
  {
    ...
    err = sdkl_npu_mm_f32f16_f32(domain, M, N, K, static_cast<float *>(Ab),
                                 ...
                                 static_cast<const _Float16 *>(Wb));
    if (err) {
      fprintf(stderr, "FAIL case3: npu_mm err=%d\n", err);
      ++failures;
      goto done;
    }
    memcpy(C_npu.data(), Ab, M * N * sizeof(float));
