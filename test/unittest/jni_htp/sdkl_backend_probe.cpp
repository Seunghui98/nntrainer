...
static void cpuGemm(const float *X, const float *W_f32, float *C) {
  for (int i = 0; i < M; ++i)
    for (int j = 0; j < N; ++j) {
      float s = 0.f;
      for (int k = 0; k < K; ++k) s += X[i*K+k] * W_f32[j*K+k];
      C[i*N+j] = s;
    }
}
...
int main() {
  ...
  // ---- Case 1: W = identity ------------------------------------------------
  {
    ...
    nntrainer::hmx::shgemm_f32f16_f32(
      1 /*ROW_MAJOR*/, false /*TransA*/, true /*TransB*/,
      M, N, K,
      1.0f, X.data(), K,
      reinterpret_cast<const _FP16 *>(W_fp16.data()), K,
      0.0f, C_npu.data(), N);
  ...
  // ---- Case 2: W = zero ----------------------------------------------------
  {
    ...
    float mx = 0.f;
    for (int i = 0; i < M*N; ++i) mx = fmaxf(mx, fabsf(C_npu[i]));
    if (mx < 0.001f) {
  ...
  // if they diverge here, the CPU reference is wrong, not the NPU.
  {
    ...
    for (int i = 0; i < N*K; ++i) W_f32_2[i] = ((i*13+5) % 100) / 200.f - 0.25f;
    for (int i = 0; i < N*K; ++i)
    ...
    // CPU HGEMM path (nntrainer::shgemm) — exactly as cpuShgemm in the backend test
    nntrainer::shgemm(
      1 /*ROW_MAJOR*/, false /*TransA*/, true /*TransB*/, M, N, K,
      1.0f, X.data(), K,
      reinterpret_cast<const _FP16 *>(W_fp16.data()), K,
      0.0f, C_shgemm.data(), N);

    float re = relErr(C_npu.data(), C_shgemm.data(), M*N);
    if (re < 0.01f) {
      fprintf(stderr, "PASS case4 npu==shgemm: rel_err=%.5f\n", re);
    } else {
      fprintf(stderr, "FAIL case4 npu!=shgemm: rel_err=%.5f  ← CPU ref or NPU mismatch\n", re);
      for (int i = 0; i < 8; ++i)
        fprintf(stderr, "  [%d] npu=%.5f shgemm=%.5f\n", i, C_npu[i], C_shgemm[i]);
      ++failures;
    }
  }
