...
TEST(MHACoreMask, PerLayerFullVsSlidingSelection) {
  ...
  EXPECT_EQ(causallm::MHACoreLayer::selectVerifyMask(&fullMask, &slidingMask,
                                                     4096),
            &slidingMask);
  ...
  EXPECT_EQ(
    causallm::MHACoreLayer::selectVerifyMask(nullptr, &slidingMask, 2),
    nullptr);
  ...
  causallm::MHACoreLayer::add_mask_and_softmax_full(
    qk_sliding, *sel_sliding, step_size, to, num_head);
  ...
  EXPECT_GT(std::abs(qk_full.getData<float>()[0] -
                     qk_sliding.getData<float>()[0]),
            0.5f);
}
